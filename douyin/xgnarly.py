"""抖音移动端 X-Gorgon (X-Gnarly) 签名算法。

移植自 TikTokDownloader (JoeanAmier/TikTokDownloader/src/encrypt/xGnarly.py)。
X-Gorgon 是字节跳动移动端 API 的请求头签名，基于 ChaCha20 流加密 + 自定义 PRNG。
配合 X-Bogus (URL 参数签名) 和 X-Khronos (时间戳) 构成完整的移动端签名体系。
"""
