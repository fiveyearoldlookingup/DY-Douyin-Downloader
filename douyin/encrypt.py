"""抖音 Web API aBogus 签名算法。

原版来自 TikTokDownloader (JoeanAmier/TikTokDownloader/src/encrypt/aBogus.py)。
核心：SM3 国密哈希 + RC4 流加密 + 自定义 Base64 编码。

提供:
- ABogus: a_bogus 签名生成器
- generate_fake_ms_token: 随机 msToken 生成
"""
...
def generate_fake_ms_token(size: int = 156) -> str:
    """生成随机 msToken（156 位字母数字混合）。

    msToken 是字节跳动的反爬参数，经测试随机字符串也可正常使用。
    """
    import string
    charset = string.digits + string.ascii_uppercase + string.ascii_lowercase
    length = len(charset) - 1
    return "".join(charset[randint(0, length)] for _ in range(size))
